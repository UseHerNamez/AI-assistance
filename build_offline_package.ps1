...
$offlineFolder = Join-Path $offlineDir "Assistance-Offline"
...
Write-Step "Preparing offline staging folder (v$ReleaseVersion)"
if (Test-Path $buildTemp) {
    Remove-Item $buildTemp -Recurse -Force
}
Ensure-Directory $assetsDir

Copy-ReleaseStage -ProjectDir $projectDir -StageDir $offlineStage -Items (Get-ReleaseOfflineItems) -WriteBuildInfo
...
Write-Step "Writing offline package marker"
Set-Content -Path (Join-Path $offlineStage "offline.package") -Value $ReleaseVersion -Encoding ASCII

if (-not $SkipFolder) {
    Write-Step "Copying Assistance-Offline folder for USB/offline transfer"
    if (Test-Path $offlineFolder) {
        Remove-Item $offlineFolder -Recurse -Force
    }
    Copy-Item -Path $offlineStage -Destination $offlineFolder -Recurse -Force
    Write-Host "Created: $offlineFolder"
}
...
if (-not $SkipZip) {
    ...
    $zipSource = if (Test-Path $offlineFolder) { Join-Path $offlineFolder "*" } else { Join-Path $offlineStage "*" }
...
$folderForSize = if (Test-Path $offlineFolder) { $offlineFolder } else { $offlineStage }
$folderMb = [math]::Round((Get-ChildItem $folderForSize -Recurse | Measure-Object -Property Length -Sum).Sum / 1MB, 1)
Write-Host ""
Write-Host "Offline package ready in dist\offline:" -ForegroundColor Green
Write-Host ("  Version: {0}" -f $ReleaseVersion)
Write-Host ("  Assistance-Offline\ folder ({0} MB)" -f $folderMb)
Write-Host "  Assistance-Offline.zip"
Write-Host "  Assistance-Offline-Setup.exe (if Inno Setup is installed)"
Write-Host ""
Write-Host "Copy dist\offline\ to a PC with NO internet."
